...
@section('conteudo')
        ...
        <div class="container">
            @foreach($permissions as $permissao)
                <div class="col-xs-2 col-sm-3 col-lg-3 col-md-3">
                    <a data-toggle="modal" data-target="#{{md5($permissao->name)}}"
                       style="color:#fff;cursor:pointer;font-size: 1.4em">
                        <i class="fa fa-rebel" aria-hidden="true"></i> {{ $permissao->name}}
                    </a>
                    <div class="espacos"></div>
                </div>
            @endforeach
    ...
    @foreach($permissions as $maestria)
        <div id="{{md5($maestria->name)}}" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">{{$maestria->name}}</h4>
                    </div>
                    <div class="modal-body">
                        <form action="{{url(getenv('ADMIN_ROUTE', true)."/".getenv('permissoes_ROUTE',true)."/".getenv('EDIT_ROUTE',true)."/"."$maestria->name/$maestria->id")}}"
                    ...
                    <div class="modal-footer">
                        <form action="{{url(getenv('ADMIN_ROUTE', true)."/maestria/deletar/$maestria->name/$maestria->id")}}"
